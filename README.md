<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:2C5364,100:2E9EF7&height=200&section=header&text=Sakib%20Ahamed%20Sihab&fontSize=42&fontColor=ffffff&fontAlignY=38&desc=Frontend%20Developer%20%7C%20React%20%2F%20Node.js&descAlignY=58&descSize=18&animation=fadeIn" width="100%"/>
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=20&pause=1200&color=2E9EF7&center=true&vCenter=true&width=650&lines=Building+clean%2C+functional+web+experiences;Currently+deep+in+Next.js%2C+routing%2C+real+projects;Aiming+for+a+job+%2F+internship" alt="Typing SVG"/>
<br/>
<a href="https://www.linkedin.com/in/sakib-ahamed-sihab/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/></a> <a href="mailto:sakiba505@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
</div>
<br/>

### 👋 About Me

Frontend developer from Bangladesh who builds full projects, not just tutorials — from UI to API. Right now I'm sharpening my React fundamentals (hooks, routing, state management) while rounding out my stack with Node.js, Express, and MongoDB so I can ship complete apps end-to-end. Currently aiming for a job or internship.

<br/>

### 🧰 Stack

<div align="center">
<img src="https://skillicons.dev/icons?i=html,css,js,ts,react,nextjs,tailwind,nodejs,express,mongodb,java,cpp&theme=dark" />
</div>
<div align="center">

`Currently learning:` **Mongoose** · **PostgreSQL**

</div>

<br/>

### 📌 Projects

<table>
<tr>
<td width="50%" valign="top">

### 🗂️ [job-tracker](https://github.com/sakibahamedsihab/job-tracker)
A simple tracker for job applications — log companies, roles, and status so nothing gets lost during a job search.
<br/>`HTML`

</td>
<td width="50%" valign="top">

### 🎯 [keen-keeper](https://github.com/sakibahamedsihab/keen-keeper)
Tracks how long it's been since you last talked to your friends, and reminds you to reach out if it's been over a month.
<br/>`JavaScript`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🐑 [qurbani-hut-v2](https://github.com/sakibahamedsihab/qurbani-hut-v2)
A livestock marketplace platform for buying and selling animals for Qurbani.
<br/>`JavaScript`

</td>
<td width="50%" valign="top">

### 🐾 [pet-adoption-client](https://github.com/sakibahamedsihab/pet-adoption-client)
A pet adoption platform where users can browse and adopt pets.
<br/>`JavaScript`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🎧 [spiral-sound-api](https://github.com/sakibahamedsihab/spiral-sound-api)
A REST API for a platform where singers/artists upload their songs and listeners pay to listen. Frontend in progress.
<br/>`JavaScript`

</td>
<td width="50%" valign="top">

### 🍳 [recipe-hub-app-client](https://github.com/sakibahamedsihab/recipe-hub-app-client)
A social-media-style app for sharing recipes — post, browse, and discover recipes from others.
<br/>`JavaScript`

</td>
</tr>
</table>

<br/>

<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:2E9EF7,100:0F2027&height=100&section=footer"/>
<img src="https://komarev.com/ghpvc/?username=sakibahamedsihab&style=for-the-badge&color=2E9EF7&label=PROFILE+VIEWS"/>
</div>
